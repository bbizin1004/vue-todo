...
//reactive함수:객체를 반응성으로 만들며, 객체의 값이 변경되면 리렌더링하여 화면에 업데이트 해줌
const data = reactive({
  newItem: '',
  items: [],
});

//computed함수: data.items가 변경될때만 함수가 실행된다.
  ...
  <main class="app">
    ...
    <ul class="todo_list">
      ...
      <li v-for="(item, index) in data.items" v-bind:key="item.id" v-bind:class="{ completed: item.completed }">
        <!-- 변수데이터와 속성견결시 v-bind를 사용해야함 -->
        <!-- 라벨클릭시 for로 연결된 체크박스가 클릭되며, 체크 되었는지 안되었는지 true,false가 발생하며 v-model로 연결된 속성에 들어감 -->
        <input v-bind:id="`check${item.id}`" v-model="item.completed" type="checkbox" />
        <label v-bind:for="`check${item.id}`">{{ item.text }}</label>
        <button v-on:click="deleteItem(item.id)" type="button" class="remove_btn">Remove</button>
      </li>
    </ul>
  </main>
</template>
